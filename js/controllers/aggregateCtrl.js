...
qdmpApp.controller(
	'AggregateCtrl',
	['$scope', '$rootScope', 'WorkflowService', 'UserService',
    function ($scope, $rootScope, WorkflowService, UserService) {
        ...
        UserService.getCurrentPermissions().success(function (permissions) {
            ...
            $scope.workflowAction = function (id) {
                $location.path('aggregateDetail/' + linkContext + '/' + id);
            };
        });
        ...
        WorkflowService.getWorkflow(null, 'AGGREGATION').success(function (workflows) {
            var final_workflows = [];
